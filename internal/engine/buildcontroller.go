...
func buildStateSet(ctx context.Context, manifest model.Manifest, specs []model.TargetSpec,
	...
	for _, spec := range specs {
		...
		// TODO(nick): There's no good reason for this 'if' statement, but lots of
		// tests rely on there not being a K8sTarget BuildState
		if id.Type != model.TargetTypeImage &&
			id.Type != model.TargetTypeDockerCompose &&
			id.Type != model.TargetTypeLocal {
			continue
		}
